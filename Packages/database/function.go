...
func CreatePlayer(s *discordgo.Session, player Struct.Player) {
	...
	if p.Username == player.Username {
		...
		err := db.QueryRow(context.Background(),
			`insert into public.players (username,level,exp,po,job_id,rank,city_id) values ($1,$2,$3,$4,$5,$6,$7)`,
			player.Username, player.Level, player.EXP, player.PO, player.Job.ID, player.Rank, player.City.ID)
		if err != nil {
		...
		err = db.QueryRow(context.Background(),
			...
			player.ID, player.Attribute.HP, player.Attribute.MP, player.Attribute.Strength, player.Attribute.Agility, player.Attribute.Intelligence, player.Attribute.Spirit, player.Attribute.Constitution, player.Attribute.Speed, player.Attribute.Luck)
		if err != nil {
		...
		err = db.QueryRow(context.Background(),
			...
			player.ID, player.DropRank.Plants, player.DropRank.Animals, player.DropRank.Minerals, player.DropRank.Magic, player.DropRank.Special)
		if err != nil {
